...
global done_count   # 用于记录下载完成的个数

async def download_unit(no, total, name, archive, client):
    # logger.debug(no)
    ...
    html = await client.get(url=video_page_url, headers=headers)
    ...
    async with aiofiles.open(f"video/{name}/{no}_{title}.flv", "wb") as f:
        await f.write(res.content)

    audio_url = data["data"]["dash"]["audio"][0]["baseUrl"]
    res = await client.get(audio_url, headers=headers)
    async with aiofiles.open(f"audio/{name}/{no}_{title}.mp3", "wb") as f:
        await f.write(res.content)
...
async def download_season(season_id, name, total, start_index=1):
    ...
    global done_count
    done_count = 0
    async with httpx.AsyncClient(timeout=None) as client:   # timeout需为None，否则很容易readTimeout
        print("下载中...")
        task_list = []
        # 太快了B站会关闭连接，所以考虑分片，每次8个一起下
        for i, (no, archive) in enumerate(archive_list):
            task_list.append(download_unit(no, total, name, archive, client))
            if i % 8 == 0:
        ...
        #     print(f"\r下载进度: {done_count}/{total}", end="\n")
...
async def main():
    ...
    for season in seasons_list:
        ...
        await download_season(season_id, name, total, 1)

        for file in os.listdir(f"./video/{name}"):
            file = file[:-4]
            merge(
                f"./video/{name}/{file}.flv",
                f"./audio/{name}/{file}.mp3",
                f"./out/{name}/{file}.flv",
            )
